...
* Using [Composite functions](#composite-functions) to allow custom programs to
  run alongside the Composition. 
* Creating a 
  [custom definition of when a resource is ready](#resource-readiness-checks) to use. 
...
defines the set of things that a Composite Resource can create.
...
The {{<hover label="resources" line="17">}}spec.forProvider{{</hover>}}
supports any settings used in a managed resource including
applying `annotations` and `labels` or using a specific 
`providerConfigRef`.

{{<hint "note" >}}
Compositions allow applying a `metadata.name` but ignores it. The
...
Compositions can use existing Composite Resources to define more complicated
templates. 
...
defines which Composite Resource types can use this Composition. 
...
{{<hover label="patch" line="15">}}location{{</hover>}} provided in a Composite 
Resource and applying it to the 
...
Composite functions (`XFNs`) are containers executing any code you define.
Composite functions can read and write to any resource in the Composition
they're attached to. This allows composite functions to preform complex
...
{{<hint "important" >}}
Composite functions are an alpha feature. Alpha features aren't enabled by
default. 
{{< /hint >}}

To attach a composite function to a Composition define a 
...
location of the composite function container image.
...
details on building and using composite functions.
...
{{<hint "note" >}}
The managed resource defines the generated details.   
Read the
[Managed Resources]({{<ref "./managed-resources#writeconnectionsecrettoref" >}})
page for more information about connection secrets.
{{</hint >}}

The Composition defines where to save the details generated by any resources.
#### Save connection details to a namespace
By default Crossplane saves connection details in the `default` namespace. A
Composition can save connection details to a specific namespace with
`writeConnectionSecretsToNamespace`. 

For example, to save all connection details to the namespace `my-namespace` set
...
Use `publishConnectionDetailsWithStoreConfigRef` to define the `StoreConfig`
