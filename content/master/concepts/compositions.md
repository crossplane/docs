...
to the namespace where Crossplane stores the combined secret object.
...
For example, a 
{{<hover label="tipPatch" line="14">}}patch{{</hover>}}
...
A Composition must define the specific keys a resource creates with 
the 
...
{{<hover label="conDeet" line="20">}}fromValue{{</hover>}}.
...
By default CompositeResourceDefinitions store all resource
keys. 
