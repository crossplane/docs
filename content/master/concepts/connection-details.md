...
{{<hint "warning">}}
When chaning the {{<hover label="xrd" line="4">}}connectionSecretKeys{{</hover>}} of an XRD the change is not immediatly reflected.
You have two options to change the keys in the combined secret object.
- Delete and recreate the XRD. This only makes sense if the XRD is not used as it will lead to the deletion of XRs.
