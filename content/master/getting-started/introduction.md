...
_Composite Resources_ (`XRs`) access custom Kubernetes APIs defined by Crossplane 
administrators.
...
{{<hint "tip" >}}
Creating _Composite Resource Definitions_ is like creating a Kubernetes
...
the values of the `spec` for a _Composite Resource_. 

The _Composite Resource Definition_ has four important sections:
* A `group` to define the {{< hover label="XR2" line="2" >}}apiVersion{{< /hover >}} in a _Composite Resource_.
* The `names` to define the {{<hover label="XR2" line="3" >}}kind{{< /hover >}}
  used in a _Composite Resource_.
* The `versions` and `versions.schema` that define the valid inputs used in a
  _Composite Resource_.
* The `claimNames` to create Crossplane _Claims_.

{{< hint "note" >}}
Claims are covered in the [Claims]({{<ref "#claims" >}}) sections.
{{< /hint >}}

Using the previous _Composite Resource_ as an example. 
...
The {{<hover label="specGroup" line="3">}}spec.group{{< /hover >}} defines the _Composite Resource_
{{<hover label="XR2" line="2">}}apiVersion{{< /hover >}}.

Next, the {{<hover label="specNames" line="5">}}spec.names{{< /hover >}} defines the _Composite Resource_
{{<hover label="XR2" line="3">}}kind{{< /hover >}}.

```yaml {label="specNames"}
# Composite Resource Definition (XRD)
spec:
  group: test.example.org
  names:
    kind: myComputeInstance
    plural: myComputeInstances
```

{{< hint "note" >}}
The `spec.names.plural` value allows `kubectl` to use either singular or plural
names in commands and outputs.
{{< /hint >}}

The {{<hover label="specVersions" line="7" >}}spec.versions{{</hover >}}
and {{<hover label="specVersions" line="11" >}}spec.schema{{</hover >}} define 
an API revision number and the valid inputs. 

This example defines a {{<hover label="specVersions" line="18" >}}size{{< /hover >}}
parameter. The size is a 
{{<hover label="specVersions" line="21">}}string{{< /hover >}} 
and the OpenAPI 
...
The schema options for _Composite Resource Definitions_ follow the OpenAPIv3
schema. 

{{<hint "tip" >}}
More details on _Composite Resource Definitions_ are available in the 
[Concepts page]({{<ref "../concepts/composition/">}}).
{{< /hint >}}
...
Like _Composite Resources_, a _Composite Resource Definition_ defines the values
a _Claim_ uses. A _Composite Resource Definition_ allows _Claims_ based on the
_Composite Resource Definition's_ `spec.claimNames` value. 

{{< hint "note" >}}
A _Composite Resource Definition_ without a `spec.claimNames` doesn't create a
Claim.
{{< /hint >}}
